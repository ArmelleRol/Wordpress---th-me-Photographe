jQuery(document).ready(function($) {  // encapsule $ car wordpress utilise plusieurs librairie jquery

  /* *************************************************************  */
  /*                  suppression d'un message                      */
  /* *************************************************************  */
  if(document.getElementById('table-messages')) {
    let contentBefore, $td, id_to_delete; // declaration des variables pour les modifier en dehors de la variable
    ...
    $("#table-messages").on('click', '.deletable', function(e) {
      ...
      if (clickable == true)  { // si variable = true on execute le code 
        clickable = false; // terminer l'opération avant de cliquer sur un autre bouton 

        e.preventDefault();
        let $this = $(this);
        id_to_delete = $this.data('id'); // stocke id 
        $td = $this.parent(); // contenu de la cellule qui contient le bouton this parent-> element supérieur 
        contentBefore = $td.html(); //contenu de la cellule td  = lien pour le restituer si annulation de la suppression
        $(".deletable").addClass('tempo').removeClass('btn btn-default');

        $td.html('');
        let stringConfirm  = "<p id='confirm-delete' >La suppression est irréversible!"; // indique les éléments html 
    ...
    // ecouteur d'evenement click sur bouton my cancel pour annuler opération - on => detecter par jquery apres qu'il a été créé
    $("#table-messages").on('click', '#my-cancel', function(e) {  
      e.preventDefault(); 
      $td.find("#confirm-delete").remove().end().html(contentBefore); // cellule td chercher element eneleve la croix et affiche  confirm delete html-> contenu de content before
    ...
    $("#table-messages").on('click', '#my-confirm', function(e) {  
      ...
      $.post(
        ajaxurl, // 
        {
          'action': 'delete_message', // parametre action delete doit correspondre au hook wp_ajax_delete_message dans le fichier function 
